...
class DATCAdjudicatorTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        try:
            cls.test_user = User.objects.get(pk=cls.TEST_USER_PK)
             ...
             raise Exception("Fixture default_user.yaml did not load User with pk=%s correctly." % cls.TEST_USER_PK)
    ...
    def _parse_unit_string(self, line):
        ...
        try:
            ...
            raise ValueError("Could not parse unit string: '%s' - Error: %s" % (line, e))
    ...
    def _parse_order_string(self, line):
        ...
        try:
            ...
            raise ValueError("Could not parse order string: '%s' - Error: %s" % (line, e))
    ...
    def _get_actual_poststate(self, game):
        ...
        for unit in Unit.objects.filter(player__game=game):
            ...
            # if unit.coast: coast_str = "/%s" % unit.coast
            # Use Country name from the related Country object
            unit_str = "%s: %s %s%s" % (unit.player.country.name, unit.type, area_code.lower(), coast_str)
    ...
    def _parse_poststate_lines(self, lines):
        ...
        for line in lines:
            try:
                ...
                units.add("%s: %s %s%s" % (country_name, unit_type, area_code.lower(), coast_str))
            except ValueError as e:
                print("Warning: Could not parse poststate line: '%s' - %s" % (line, e))
        return units
    ...
    def _create_game_state(self, case_data):
        ...
        game_slug = "datc-%s-%s" % (case_data['name'].replace(' ', '-').replace('.', '_').lower(), os.urandom(4).hex())
        ...
        if case_data['pre_phase']:
             try:
                 ...
                 raise ValueError("Could not parse PRESTATE_SETPHASE '%s': %s" % (case_data['pre_phase'], e))
        ...
        for unit_line in case_data['pre_units']:
            ...
            if not country or not area:
                raise ValueError("Fixture Error: Unknown country '%s' or area '%s' in line: %s" % (country_name, area_code, unit_line))
            ...
            if not game_area:
                 raise ValueError("Could not find GameArea for Area %s in game %s" % (area_code, game.id))
            ...
            unit_key = "%s: %s %s%s" % (country_name, unit_type, area_code.lower(), "/%s" % coast if coast else "")
            ...
            for unit_line in case_data['pre_dislodged']:
                 ...
                 if unit_obj:
                     ...
                     unit_obj.save()
                      ...
                      print("Warning: Could not find unit '%s' to mark as dislodged." % unit_key)
    ...
    def _create_orders(self, game, players, units_map, order_lines):
        ...
        for order_line in order_lines:
            try:
                ...
                unit_key = "%s: %s %s%s" % (country_name, unit_type, unit_area_code.lower(), "/%s" % unit_coast if unit_coast else "")
                unit_obj = units_map.get(unit_key.strip())
                if not unit_obj: # Try without coast as fallback
                     unit_key_base = "%s: %s %s" % (country_name, unit_type, unit_area_code.lower())
                     unit_obj = units_map.get(unit_key_base.strip())

                if not unit_obj:
                    raise ValueError("Could not find unit '%s' from PRESTATE for order: %s" % (unit_key, order_line))
                ...
                if verb == 'H' or verb == 'HOLDS':
                    ...
                    if not dest_game_area: raise ValueError("Unknown destination area code '%s'" % dest_area_code)
                    order_data['code'] = '-'
                    ...
                    if not supported_unit_obj: raise ValueError("Cannot find supported unit %s %s" % (sup_unit_type, sup_unit_area_code))
                    ...
                    if len(details) > 3:
                        ...
                        if sup_action == '-': # Support Move
                            ...
                            if not sup_dest_game_area: raise ValueError("Unknown support destination '%s'" % sup_dest_area_code)
                    ...
                    if not convoyed_unit_obj: raise ValueError("Cannot find convoyed unit %s %s" % (conv_unit_type, conv_unit_area_code))
                    ...
                    if not conv_dest_game_area: raise ValueError("Unknown convoy destination '%s'" % conv_dest_area_code)
                ...
                # Add other verbs if needed (Build, Remove, Retreat)
                     ...
                     print("Skipping BUILD order: %s" % order_line)
                     continue
                elif verb == 'REMOVE' or verb == 'DISBAND':
                     # Requires Adjustment phase logic - Skip for movement tests
                     print("Skipping REMOVE/DISBAND order: %s" % order_line)
                     continue
                     ...
                     if game.phase == PHRETREATS:
                         ...
                         if not retreat_dest_game_area: raise ValueError("Unknown retreat destination '%s'" % retreat_dest_code)
                         # Create RetreatOrder instead of Order
                         RetreatOrder.objects.create(unit=unit_obj, area=retreat_dest_game_area)
                         print("Created RetreatOrder: %s -> %s" % (unit_obj, retreat_dest_game_area))
                         continue # Skip creating regular Order
                     else:
                         print("Skipping RETREAT order in non-retreat phase: %s" % order_line)
                         continue
                ...
                print("Error processing order line: '%s' - %s" % (order_line, e))
    ...
    def _run_datc_test(self, case_data):
        ...
        print("\nRunning DATC Case: %s" % case_data['name'])
        ...
        if game.phase == PHORDERS:
            try:
                ...
                if is_poststate_same:
                     print("Order creation failed as expected for invalid order test: %s" % e)
                     ...
                     raise AssertionError("Order creation failed unexpectedly for case %s: %s" % (case_data['name'], e))
             ...
             for order_line in case_data['orders']:
                 try:
                     ...
                     unit_key = "%s: %s %s%s" % (country_str.strip(), unit_type, unit_area_code.lower(), "/%s" % unit_coast if unit_coast else "")
                     unit_obj = units_map.get(unit_key.strip())
                     if not unit_obj: # Fallback without coast
                          unit_key_base = "%s: %s %s" % (country_str.strip(), unit_type, unit_area_code.lower())
                          unit_obj = units_map.get(unit_key_base.strip())

                     if not unit_obj: raise ValueError("Cannot find unit '%s' for retreat." % unit_key)
                     ...
                     if action == 'DISBAND':
                         ...
                         if not dest_game_area: raise ValueError("Unknown retreat destination '%s'" % dest_code)
                         RetreatOrder.objects.create(unit=unit_obj, area=dest_game_area)
                         # TODO: Handle retreat coast if model supports it
                 except Exception as e:
                     print("Error processing RETREAT order line: '%s' - %s" % (order_line, e))
                     raise
...
try:
    ...
    for case_data in all_datc_cases:
        test_method_name = "test_case_%s" % case_data['name'].replace('.', '_').replace(' ', '_').replace('-', '_').replace('(', '').replace(')', '').lower()
    ...
    print("Warning: DATC file not found at %s. Skipping DATC tests." % DATC_FILE_PATH)
except Exception as e:
    print("Error setting up DATC tests: %s" % e)